...
namespace HoLLy.Memory.Scanning
{
    public static class PatternScanner
    {
        ...
        public static bool Scan(Process proc, PatternByte[] pattern, bool? mapped, IntPtr entry, out UIntPtr result)
        {
            result = default;

            var i = proc.GetMemoryRegions().Where(x => !mapped.HasValue || x.IsMapped == mapped.Value);

            foreach (WindowsMemoryRegion region in i) {
                if (region.BaseAddress > (ulong) entry)
                    if (Scan(proc, pattern, (UIntPtr) region.BaseAddress, region.RegionSize, out result))
                        return true;
            }
